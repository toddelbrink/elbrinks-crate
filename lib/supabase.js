...
//
// source='backfill' events are excluded — they're synthetic historical
// placeholders with mood_active=null by definition, but filtering by source
// is defensive in case future backfills carry mood data we don't want in
// pattern aggregations.
async function loadMoodEventCounts({ userId, since } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('mood_active, source');
    if (userId) q = q.eq('user_id', userId);
...
// Multi-mood plays contribute to each slug they tag. Backfill events excluded.
// Null mood plays skipped (no mood to anchor to).
async function loadMostPlayedByMood({ userId } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('release_id, mood_active, source');
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) throw error;
    const perMood = {};
...
// (the most forgotten case). Backfill events excluded. Null mood plays skipped.
async function loadMoodLastPlayed({ userId } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('played_at, mood_active, source');
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) throw error;
    const last = {};
