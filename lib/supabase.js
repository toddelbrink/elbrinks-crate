  ...
  try {
    ...
    for (const row of data) {
      map[row.release_id] = {
        ...
        moods_source: row.moods_source || 'manual',
        // Sprint 3 §13 — Liner Notes columns. Mapper must include these
        // explicitly even though the underlying query is select('*').
        liner_notes: Array.isArray(row.liner_notes) ? row.liner_notes : null,
        liner_notes_status: row.liner_notes_status || 'pending',
        liner_notes_generated_at: row.liner_notes_generated_at || null,
      };
    }
    return ok({ data: map });
  } catch (e) { return fail(e, { data: {} }); }
...
// Anon-readable subset (cycle, theme) via public_settings view. Pass the
// share page owner's userId; without it, rows from every share-enabled user
// come back and the last one wins.
async function loadPublicSettings({ userId } = {}) {
  try {
    let q = supabase.from('public_settings').select('setting_key, setting_val');
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) throw error;
    const map = {};
...
async function saveCollection(records) {
  ...
  try {
    // Dedupe by release_id, last wins. Two reasons this is required, not tidy:
    // (1) Postgres rejects an upsert whose batch hits the same conflict target
    //     twice ("ON CONFLICT DO UPDATE command cannot affect row a second
    //     time"), so a duplicate would fail the whole write. Owning two copies
    //     of the same pressing is enough to trigger it.
    // (2) The count assertion below compares sent against returned, and the DB
    //     collapses duplicates — so a duplicate would read as a partial write.
    const byId = new Map();
    for (const r of records) {
      const rid = r.releaseId || r.id;
      if (!rid) continue;
      byId.set(String(rid), { release_id: String(rid), data: r });
    }
    const rows = [...byId.values()];
    const { data, error } = await supabase.from('vinyl_collection').upsert(rows).select('release_id');
    if (error) throw error;
    // Defense in depth per supabase_patterns.md §14 — assert the write actually
    // landed instead of trusting the absence of an error. A partial or dropped
    // write has to surface here, not as a user discovering an empty crate two
    // months later (P0 Defect 2: John shipped 187 meta rows and 0 collection rows).
    const written = (data || []).length;
    if (written !== rows.length) {
      throw new Error(`partial collection write: ${written}/${rows.length} rows persisted`);
    }
    return ok({ message: `Saved ${rows.length} records` });
  } catch (e) { return fail(e); }
}

// removeFromCollection — delete the current user's vinyl_collection rows for
// exactly these release_ids. Upserts never delete, so a record removed on
// Discogs used to stay here (and on the public share page) forever. Takes an
// explicit list rather than a keep-list so a caller can only remove rows it has
// already inspected. vinyl_meta / vinyl_plays are left alone: moods, notes, and
// play history come back if the record is re-added.
async function removeFromCollection(releaseIds) {
  const ids = [...new Set((releaseIds || []).map(String).filter(Boolean))];
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('release_id');
    if (error) throw error;
    return ok({ data: (data || []).map(r => r.release_id) });
  } catch (e) { return fail(e); }
}
...
async function setPlayCount(releaseId, count) {
  ...
  try {
    ...
    // cycle_played always derives from remaining cycle-event count, not from
    // the previous flag value. Pencil never creates events, so decrement-trim
    // can drive cycle_played from true→false. Source-of-truth is events.
    // Session 40 CC fix for phantom dim (vinyl_plays.cycle_played=true with
    // zero matching events) caught by Todd's pencil-down test.
    const { data: profile } = await supabase
      .from('vinyl_user_profile')
      .select('cycle_started_at')
      .maybeSingle();
    const cycleStart = profile?.cycle_started_at || null;
    let cycleQuery = supabase
      .from('vinyl_play_events')
      .select('id', { count: 'exact', head: true })
      .eq('release_id', String(releaseId));
    if (cycleStart) cycleQuery = cycleQuery.gte('played_at', cycleStart);
    const { count: cycleEventCount, error: cqErr } = await cycleQuery;
    if (cqErr) throw cqErr;
    const cyclePlayed = (cycleEventCount || 0) > 0;
    // Preserve existing last_played when pencil-incrementing (admin back-fill
    // of an old play shouldn't update last_played to now); set to now() only
    // when this is the first row (no existing) or when we have events to
    // anchor on. Keep prior behavior of stamping now() for new rows.
    const last = existing?.last_played || new Date().toISOString();
    const { data, error } = await supabase
      .from('vinyl_plays')
      .upsert({
        release_id: String(releaseId),
...
// resetCycle — flips every owned vinyl_plays.cycle_played to false AND stamps
// vinyl_user_profile.cycle_started_at to the moment of reset. The timestamp
// is the boundary that loadCyclePlayCounts uses to count this-cycle events.
// Returns the new boundary so the caller can update its in-memory cache
...
async function resetCycle() {
  try {
    ...
    const { error: profErr } = await supabase
      .from('vinyl_user_profile')
      .update({ cycle_started_at: startedAt, updated_at: startedAt })
      .eq('user_id', session.user.id);
    if (profErr) throw profErr;
    return ok({ message: 'Cycle reset', data: { cycle_started_at: startedAt } });
  } catch (e) { return fail(e); }
}

// loadMoodEventCounts — returns {slug: count} of vinyl_play_events grouped by
// mood_active slug, optionally filtered by since (cycle_started_at, 30 days ago,
// or null for all time). Powers the Mood Patterns sub-page tiles (donut + bar).
//
// Multi-mood plays contribute 1 to EACH slug they tag — per PRD §9.2/§9.5,
// pattern aggregation handles multi-mood noise statistically.
//
// Plays with mood_active=null (shuffle-all in v1, or untagged albums under PULL)
// are skipped — they're not anchors to any mood bucket.
//
// All sources included (Session 40 CC, Option B). Backfill events that carry
// mood signal (populated post-hoc from vinyl_meta.moods) participate in mood
// aggregations. Null-mood backfill stays excluded via the mood_active check.
async function loadMoodEventCounts({ userId, since } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('mood_active');
    if (userId) q = q.eq('user_id', userId);
    ...
    return ok({ data: { counts, totalPlays } });
  } catch (e) { return fail(e, { data: { counts: {}, totalPlays: 0 } }); }
}

// loadCyclePlayCounts — returns a {release_id: count} map of vinyl_play_events
// rows since the given boundary. Used by album detail to render "This cycle N"
// alongside the lifetime play_count.
//
// since=null is treated as "count everything" (new users with no boundary set
// yet — every event is in their first cycle).
//
// Note: this reflects EVENT count, not play_count. They can diverge:
//   - pencil +/- bumps play_count without writing events (per Session 36 CC
//     locked decision — admin correction, not a play)
//   - synthetic 'backfill' events count too (one per gap album, see
//     v1_1_play_events_backfill_for_pre_step5_cycle_plays migration)
// Both divergences are by design; the events table is the truth-teller of
// real plays.
async function loadCyclePlayCounts({ userId, since } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('release_id');
    if (userId) q = q.eq('user_id', userId);
    if (since) q = q.gte('played_at', since);
    const { data, error } = await q;
    if (error) throw error;
    const counts = {};
    for (const row of data || []) {
      counts[row.release_id] = (counts[row.release_id] || 0) + 1;
    }
    return ok({ data: counts });
  } catch (e) { return fail(e, { data: {} }); }
}

// loadMostPlayedByMood — returns {[slug]: {release_id, count}} of the top
// played release per mood. Powers Mood Patterns Tile 5 (Most-Played per Mood).
// Multi-mood plays contribute to each slug they tag. All sources included
// (Session 40 CC, Option B). Null mood plays skipped (no mood to anchor to).
async function loadMostPlayedByMood({ userId } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('release_id, mood_active');
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) throw error;
    const perMood = {};
    for (const row of data || []) {
      ...
      for (const slug of moods) {
        ...
        bucket[row.release_id] = (bucket[row.release_id] || 0) + 1;
      }
    }
    const top = {};
    for (const slug of Object.keys(perMood)) {
      const entries = Object.entries(perMood[slug]).sort((a, b) => b[1] - a[1]);
      if (entries.length) top[slug] = { release_id: entries[0][0], count: entries[0][1] };
    }
    return ok({ data: top });
  } catch (e) { return fail(e, { data: {} }); }
}

// loadMoodLastPlayed — returns {[slug]: ISO_string} of max played_at per mood.
// Powers Mood Patterns Tile 2 (Forgotten Mood). Moods with zero qualifying
// events are omitted from the result — caller treats absence as "never played"
// (the most forgotten case). All sources included (Session 40 CC, Option B).
// Null mood plays skipped.
async function loadMoodLastPlayed({ userId } = {}) {
  try {
    let q = supabase.from('vinyl_play_events').select('played_at, mood_active');
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) throw error;
    const last = {};
    for (const row of data || []) {
      const moods = row.mood_active;
      if (!Array.isArray(moods) || !moods.length) continue;
      const t = row.played_at;
      if (!t) continue;
      for (const slug of moods) {
        if (!slug) continue;
        if (!last[slug] || new Date(t) > new Date(last[slug])) last[slug] = t;
      }
    }
    return ok({ data: last });
  } catch (e) { return fail(e, { data: {} }); }
...
async function recordPlayEvent(releaseId, opts = {}) {
  ...
  try {
    ...
    // Defense-in-depth per supabase_patterns.md §14: .select() lets us assert
    // the insert actually landed. Catches silent RLS denials + any future
    // case where the row gets rejected post-policy-check without an error.
    const { data, error } = await supabase
      .from('vinyl_play_events')
      .insert({ release_id: releaseId, played_at: new Date().toISOString(), mood_active: mood, source })
      .select('id');
    if (error) return fail(error);
    if (!data || data.length === 0) return fail(new Error('insert returned 0 rows (silent denial?)'));
...
async function loadProfile() {
  try {
    const { data, error } = await supabase
      .from('vinyl_user_profile')
      .select('crate_name, share_enabled, share_slug, telemetry_consent, onboarding_completed_at, cycle_started_at, auto_suggest_enabled, liner_notes_enabled')
      .maybeSingle();
    if (error) return fail(error);
...
async function loadProfileBySlug(slug) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('user_id, share_slug, crate_name, cycle_started_at')
      .eq('share_slug', slug)
      .maybeSingle();
    if (error) return fail(error);
    return ok({ data: data || null });
...
// loadPublicMoods — anon-safe mood vocabulary for a share page via the
// public_moods view (share_page_parity migration). Exposes slug/name/color/
// order only; keywords and description stay owner-only on vinyl_user_moods.
async function loadPublicMoods({ userId } = {}) {
  try {
    let q = supabase
      .from('public_moods')
      .select('slug, mood_name, color_key, sort_order')
      .order('sort_order', { ascending: true });
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
...
// loadPublicPlayEvents — anon-safe play history for a share page via the
// public_play_events view, newest first. The share page derives Mood Patterns,
// the Recent timeline, and "This cycle N" from this one read client-side.
async function loadPublicPlayEvents({ userId } = {}) {
  try {
    let q = supabase
      .from('public_play_events')
      .select('release_id, played_at, mood_active')
      .order('played_at', { ascending: false });
    if (userId) q = q.eq('user_id', userId);
    const { data, error } = await q;
    if (error) return fail(error, { data: [] });
...
// ── Generate liner notes (Sprint 3 §13 / Step 2 + 4) ─────────
// POSTs to /api/liner-notes with the user's session JWT. The Vercel function
// calls Claude Sonnet, validates against the 6-category enum, enforces the
// 0.7 confidence floor server-side, writes results into vinyl_meta, logs
// the call into vinyl_liner_notes_calls (rate-limit + audit).
//
// Returns:
//   { notes: [{category, body, confidence}], status, generated_at, cached }
//
// status is one of: 'generated' | 'low_confidence' | (never 'pending'|'failed'
// in a successful response; failures surface as fail() with .rateLimited
// when applicable).
async function generateLinerNotes({ release_id, artist, title, year, genres, styles }) {
  if (!release_id) return fail('release_id required');
  try {
    const session = await getSession();
    if (!session || !session.access_token) return fail('not signed in');
    const url = buildApiUrl('liner-notes');
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
window.crate = {
  ...
  // writes
  saveSetting,
  saveCollection, removeFromCollection,
  ...
  loadPublicMoods, loadPublicPlayEvents,
  // play events (v1 / Step 5)
  recordPlayEvent, loadRecentPlays,
  // cycle counts from events (v1.1 / Sprint 3 pre-Step-4 polish)
  loadCyclePlayCounts,
  // mood event aggregation for Mood Patterns sub-page (v1.1 / Sprint 3 Step 6.1)
  loadMoodEventCounts,
  // mood patterns Tiles 2 + 5 (v1.1 / Sprint 3 Step 6.2)
  loadMostPlayedByMood, loadMoodLastPlayed,
  // feedback (v1 / Step 6)
  submitFeedback, loadAllFeedback, isAdmin, loadUserEmails,
  // suggest moods (v1 / Step 7b)
  suggestMoods, generateLinerNotes,
